...
// Récupération des variables pour être compatible PHP 4.2.0
...
$pdf->SetTitle('Génération de papiers spéciaux - Lycée TURGOT - Limoges');
$pdf->SetMargins(0,0);
...
switch ($_REQUEST['papier']) {
	...
	case 1: 
	  for($j=1;$j<=$nb_papier;$j++) {
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Papier quadrillé 5mm x 5mm - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier millimettré
	case 2:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Papier millimettré - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier semi-log a 3 modules
	case 3:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Papier semi-logarithmique à 3 modules - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier semi-log a 4 modules
	case 4:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Papier semi-logarithmique à 4 modules- Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier log-log a 3 modules
	case 5:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(15,290,'Papier logarithmique-logarithmique à 3 modules - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',185,285,10);
	  }
	  break;

	// Génération papier log-log a 4 modules
	case 6:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(15,290,'Papier logarithmique-logarithmique à 4 modules - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',185,285,10);
	  }
	  break;
	...
	case 7:
	  for($j=1;$j<=$nb_papier;$j++) {
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Grille au pas 2,54mm - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier ligne d'ecriture CP N°1
	case 8:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Feuille d\'écriture - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;

	// Génération papier ligne d'ecriture 12mm
	case 9:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Feuille d\'écriture 12mm - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;
	  
	// Génération papier ligne d'ecriture 10mm
	case 10:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Feuille d\'écriture 10mm - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;
	  
	// Génération papier ligne d'ecriture 8mm
	case 11:
	  for($j=1;$j<=$nb_papier;$j++) {
		$pdf->AddPage();
		...
		if ($imp_type_papier) $pdf->Text(10,290,'Feuille d\'écriture 8mm - Généré le '.$date.' - Lycée Turgot - Limoges');
		if ($imp_logo) $pdf->Image('logo_turgot.png',190,285,10);
	  }
	  break;
	...
	default:
		...
		$pos = 105-($largeur/2);
		$pdf->Text($pos,75,'Imprimez ce document, puis déposez-le dans mon casier.');
		$pdf->Image('logo_mel.png',30,90,20);
		$pdf->Image('logo_mel.png',160,90,20);
		$largeur =  $pdf->GetStringWidth('Sinon sauvegardez ce document et envoyez-le à :');
		$pos = 105-($largeur/2);
		$pdf->Text($pos,100,'Sinon sauvegardez ce document et envoyez-le à :');
