...
-----------------------------------------------------------------
--
-- $FILE: generic_real_arrays-array_io.adb
...
package body GENERIC_REAL_ARRAYS.ARRAY_IO is
  ...
  begin
    Ada.Text_IO.Put("(");
    Float_Element_IO.Put(v(v'First));
    
    for I in v'First+1..v'last loop
      Ada.Text_IO.New_Line;
      Ada.Text_IO.Put(" ");
      Float_Element_IO.Put(v(I));
    end loop;
    Ada.Text_IO.Put(")");
    Ada.Text_IO.New_Line;
  end Put;

  procedure Put(m: in REAL_MATRIX) is
  begin
    Ada.Text_IO.Put("[");
    Float_Element_IO.Put(m(m'First(1),m'First(2)));
    for C in m'First(2)+1..m'Last(2) loop
      Ada.Text_IO.Put(" ");
      Float_Element_IO.Put(m(m'First(1),C));
    end loop;
