...
# set SRQ operation
gpib.write('DESE 1') #gpib.write(scope,'DESE 1')
gpib.write('*ESE 1') #gpib.write(scope,'*ESE 1')
gpib.write('*SRE 32') #gpib.write(scope,'*SRE 32')
...
gpib.write('head 0') #gpib.write(scope,'head 0')

# for ASCII dump, send 'dat ASCII' instead
gpib.write('dat INIT') #gpib.write(scope,'dat INIT')
sleep(sleeptime)

# get instrument settings
gpib.write('ch1:scale?') #gpib.write(scope,'ch1:scale?')
sleep(sleeptime)
voltsdiv = float(gpib.read()) #voltsdiv = float(gpib.read(scope,80)) # TOFIX 80
if voltsdiv >= 1:
	volt_string = '%i V / div' % (voltsdiv)
else:
	volt_string = '%i mV / div' % (voltsdiv * 1000)

gpib.write('hor:mai:sca?') #gpib.write(scope,'hor:mai:sca?')
sleep(sleeptime)
tmp = float(gpib.read()) #tmp = float(gpib.read(scope,80)) # TOFIX 80
rawsweep = tmp
if tmp >= 1:
	sweep_val = tmp
	sweep_suf = "s"
...
# acquire
gpib.write('acquire:state on') #gpib.write(scope,'acquire:state on')
sleep(sleeptime)

# get the waveform preamble
gpib.write('wfmpre?') #gpib.write(scope,'wfmpre?')
sleep(sleeptime)
tmp = gpib.read() #tmp = gpib.read(scope,256) # TOFIX 256
preamble = split(tmp,';')
...
# get the curve
gpib.write('curv?') #gpib.write(scope,'curv?')
sleep(sleeptime)

# binary data read and convert to list
#tmp = gpib.readbin() #tmp = gpib.readbin(scope,4096) # TOFIX 4096
raw_data = gpib.read()
tmp = raw_data
...
tmp = 9.9E37
gpib.write('measu:imm:typ PK2;:measu:imm:sou CH1') #gpib.write(scope,'measu:imm:typ PK2;:measu:imm:sou CH1')
sleep(sleeptime)
gpib.write('measu:imm:val?') #gpib.write(scope,'measu:imm:val?')
sleep(sleeptime)
tmp = float(gpib.read()) #tmp = float(gpib.read(scope,80)) # TOFIX 80
if tmp != 9.9E37:
...
gpib.write('measu:imm:typ MEAN;:measu:imm:sou CH1') #gpib.write(scope,'measu:imm:typ MEAN;:measu:imm:sou CH1')
sleep(sleeptime)
gpib.write('measu:imm:val?') #gpib.write(scope,'measu:imm:val?')
tmp = float(gpib.read()) #tmp = float(gpib.read(scope,80))
if tmp != 9.9E37:
	mean_string = 'Mean: %.3f V' % (tmp)
else: mean_string = ''

gpib.write('measu:imm:typ PERI;:measu:imm:sou CH1') #gpib.write(scope,'measu:imm:typ PERI;:measu:imm:sou CH1')
sleep(sleeptime)
gpib.write('measu:imm:val?') #gpib.write(scope,'measu:imm:val?')
tmp = float(gpib.read()) #tmp = float(gpib.read(scope,80))
if tmp >= 1:
...
gpib.write('measu:imm:typ FREQ;:measu:imm:sou CH1') #gpib.write(scope,'measu:imm:typ FREQ;:measu:imm:sou CH1')
sleep(sleeptime)
gpib.write('measu:imm:val?') #gpib.write(scope,'measu:imm:val?')
tmp = float(gpib.read()) #tmp = float(gpib.read(scope,80))
if tmp < 1e3:
