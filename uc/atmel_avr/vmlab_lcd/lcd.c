...
/*************************************************************************
	Rempli le LCD avec le caractère
		s : le caractère de remplissage
		delay : la durée entre 2 caractères (0 à 255)
		Sens : le sens du remplissage (1 = Gauche à Droite)
*************************************************************************/
...
void lcd_ChargeAccentsFrancais(void)
	{
		...
		// Charge 8 caractères perso dans la CGRAM
...
/*Dans certaine configuration, un bug apparait : ajout de qqs caractères en fin de texte puis supression de ceux-ci.
...
* Il ne c'est pas produit dans d'autres configurations et je ne l'ai pas encore trouvé*/
		...
		for (xDebut=0;xDebut<x;xDebut++)//Effacement du début de l'écran
			{
				lcd_gotoxy(xDebut,y);
				lcd_putc(' ');
			}
		...
		if ((NbCar+x)>LCD_DISP_LENGTH)
			{
				lcd_gotoxy(x,y);
				xDebut=x;
				for(Cpt=0;Cpt< (NbCar-LCD_DISP_LENGTH+1+x+xfinal);Cpt++  )// Nombre de fois ou l'on écris la chaine
					{
						lcd_gotoxy(xDebut,y);
						Decal=x-xDebut; //Pour le début
						if (Cpt>(NbCar-LCD_DISP_LENGTH+x-1) )  Fin--;// si dernier caractère dans l'écran
 						for(Cpt2=0 ;Cpt2<(LCD_DISP_LENGTH-xDebut) ;Cpt2++  ) // Ecrire la chaine de caractère
							{
