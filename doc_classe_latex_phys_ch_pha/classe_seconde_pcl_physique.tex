\classe[Seconde option PCL - Partie Physique]{Seconde\ \\
option Physique et Chimie\ \\
de Laboratoire\ \\
Partie Physique}

\chapitre{Courant et tension électrique}
\inclure{elec/cours_elec_intro} % cours elec circuit électrique

\inclure{elec/tp_i_u} % tp mesure de tension et d'intensité

\inclure{elec/fiche_methode_montage} % document fiche
    % méthode montage
    % utilisation d'un multimètre, ...

\inclure{elec/tp_loi_ohm} % tp loi d'ohm

\inclure{elec/tp_asso_r} % tp associations de R

% association r série (tracé U en fonction de I et montrer qu'on ajoute U)
% association r parallèle (tracé U en fonction de I et montrer qu'on ajoute I)

%\chapitre{Générateurs et récepteurs}
%\inclure{elec/cours_elec_gene_recep} % cours elec générateurs, récepteurs

%\inclure{elec/tp_carac_gene} % tp caractéristique d'un générateur
%\inclure{elec/tp_carac_recep} % tp caractéristique d'un
                                % récepteur (électrolyseur)


\inclure{elec/tp_puissance_energie} % tp puissance énergie (continu)



\chapitre{Optique géométrique}
\inclure{opt/tp_reflexion_refraction}

\inclure{opt/cours_lentilles_minces}
\inclure{opt/doc_constructions_lentilles}
\inclure{opt/tp_lentilles_minces_conv_conjug} % Lentilles (Images, rel
                                % de conjugaison)
\inclure{opt/tp_loupe}

\inclure{ds_2005_2006_2nde_pcl/interro_opt_lcv}
\inclure{ds_2005_2006_2nde_pcl/ds_opt_lcv}

\inclure{opt/tp_lentilles_minces_conv_foco} % Focométrie lentilles
                                % minces

% lunette astronomique





\chapitre{Mécanique}
%\inclure{meca/tp_poids_ressort_archimede}
\inclure{meca/tp_poids}
\inclure{meca/tp_ressort}
\inclure{meca/tp_archimede}

\inclure{meca/tp_equilibre_solide_3_forces}
\inclure{meca/doc_rapporteurs}

\inclure{meca/tp_moment_force_axe}

\inclure{meca/tp_poulies_palans}




%\chapitre{Thermique}


% à ajouter si le dernier document contient un nb impair de pages
%\newpage